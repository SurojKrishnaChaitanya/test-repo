...
export const SimulatorChat = ({ riskData }) => {
  ...
  useEffect(() => {
    ...
    const { dominantDriver } = calculateFeatureAttributions(simulatorParameters);

    let insight = `Model recalculated frame: ${hazardType} detected at ${riskScore}/100 risk score (Confidence: ${
      confidence ? (confidence * 100).toFixed(0) : '--'
    }%). Primary XAI Catalyst: ${dominantDriver.name} (${dominantDriver.percentage}% attribution).`;
    
    if (riskScore > 70) {
      insight += ' Critical operational thresholds breached. Severe convective hazard alert triggered.';
    } else if (riskScore > 40) {
      insight += ' Elevated convective potential. Monitor moisture flux and boundary layer buoyancy.';
    }
  ...
  const handleSend = (textToSend) => {
    ...
    setTimeout(() => {
      setIsThinking(false);
      const { dominantDriver, narrative } = calculateFeatureAttributions(simulatorParameters);
      let responseText = '';
      const lower = query.toLowerCase();

      if (lower.includes('primary') || lower.includes('driver') || lower.includes('explain')) {
        responseText = `Primary Convective Catalyst: ${dominantDriver.name} accounts for ${dominantDriver.percentage}% of the total predictive attribution weight. ${narrative}`;
      } else if (lower.includes('wind')) {
        responseText = `Wind Speed Sensitivity: Elevated kinematic winds increase lower-tropospheric shear and convergence vectors. Coupled with ${dominantDriver.name} (${dominantDriver.percentage}% attribution), this accelerates convective cell organization.`;
      } else if (lower.includes('plan') || lower.includes('response')) {
        responseText = `Operational Advisory: Given that ${dominantDriver.name} is dominant at ${dominantDriver.percentage}% attribution weight, NDMA protocol recommends pre-positioning SDRF/NDRF rescue assets and issuing localized nowcast warnings for downstream catchments.`;
      } else {
        responseText = `Neural Attribution Insight: Model sensitivity currently ranks ${dominantDriver.name} as the leading driver (${dominantDriver.percentage}% attribution). ${narrative}`;
      }

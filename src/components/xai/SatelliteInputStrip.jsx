...
import { satelliteFramesData, satelliteSource } from '../../data/satelliteFramesData';

export default function SatelliteInputStrip({ summary, region }) {
  const eventName = region ? `${region.name}, ${region.state} Atmospheric Inflow` : satelliteSource.event;
  return (
    <div className="p-6 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-slate-800 text-sm uppercase tracking-wider">
            Satellite & Reanalysis Multi-Sensor Stream
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_1fr] gap-4">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {satelliteFramesData.map((frame) => (
            <div key={frame.id} className="rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
              <img
                src={frame.imagePath}
                alt={`${eventName} — ${frame.label}`}
                className="w-full h-auto object-contain"
              />
              <div className="p-3 border-t border-slate-200">
                <p className="text-xs font-bold text-slate-800">{frame.label} IST</p>
      ...
      <div className="pt-1 text-[11px] text-slate-400">
        Scale: 0–{satelliteSource.scaleMax} {satelliteSource.scaleUnit} · Focus: {eventName} · Resolution: {satelliteSource.spatialResolution} ({satelliteSource.crs})
      </div>
    </div>
  );
}
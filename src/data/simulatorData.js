...
// telemetryData.current), not absolute values — this keeps the grid generic
...
// Baseline absolute values per region, sourced directly from telemetryData
function getBaseline(regionId) {
  const telemetry = telemetryData[regionId]?.current;
  return {

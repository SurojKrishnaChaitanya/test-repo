...
import HistoricalValidationView from '../components/validation/HistoricalValidationView';
...
export default function HistoricalPage() {
  const [activeTab, setActiveTab] = useState('validation'); // 'validation' | 'archive'
  const [selectedEvent, setSelectedEvent] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [hazardFilter, setHazardFilter] = useState('all');
  const [outcomeFilter, setOutcomeFilter] = useState('all');

  const filteredEvents = historicalData.events.filter((event) => {
  ...
  return (
    <div className="relative h-full overflow-y-auto bg-slate-50/70 p-6">
      ...
      <div className="relative z-10 space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Historical Meteorological Validation</h2>
            <p className="mt-1 text-sm text-slate-500">
              August 19–21, 2020 ground-truth benchmark and quantitative model verification.
            </p>
          </div>
          ...
          <div className="flex items-center gap-1.5 p-1 bg-white rounded-xl border border-slate-200 shadow-xs">
            ...
            >
              ...
              <span>Aug 19–21 Benchmark Split-Screen</span>
            </button>
            ...
            >
              ...
              <span>Archived Event Matrix</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur-sm transition-shadow hover:shadow-md">
            ...
            <div className="mt-2 flex items-end justify-between">
              <p className="text-2xl font-bold text-slate-900">
                {historicalData.kpis.modelPrecision.value}%
              </p>
              <span className="text-xs font-semibold text-emerald-600">
                +{historicalData.kpis.modelPrecision.yoyChange}% YoY
              </span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur-sm transition-shadow hover:shadow-md">
            ...
            <div className="mt-2 flex items-end justify-between">
              <p className="text-2xl font-bold text-slate-900">
                {historicalData.kpis.recallRate.value}%
              </p>
              <span className="text-xs font-semibold text-emerald-600">
                +{historicalData.kpis.recallRate.yoyChange}% YoY
              </span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur-sm transition-shadow hover:shadow-md">
            ...
            <div className="mt-2 flex items-end justify-between">
              <p className="text-2xl font-bold text-slate-900">
                {historicalData.kpis.avgLeadTime.value}
                {historicalData.kpis.avgLeadTime.unit}
              </p>
              <span className="text-xs font-semibold text-emerald-600">
                +{historicalData.kpis.avgLeadTime.yoyChange} hrs YoY
              </span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur-sm transition-shadow hover:shadow-md">
            ...
            <div className="mt-2 flex items-end justify-between">
              <p className="text-2xl font-bold text-slate-900">
                {historicalData.kpis.falseAlarmRate.value}%
              </p>
              <span className="text-xs font-semibold text-emerald-600">
                {historicalData.kpis.falseAlarmRate.yoyChange}% YoY
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200/80 bg-white/90 p-5 shadow-sm backdrop-blur-sm">
          ...
          <div className="mt-5">
            <MonthlyEventChart data={historicalData.monthlyFrequency} />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200/80 bg-white/90 p-5 shadow-sm backdrop-blur-sm">
          ...
          <div className="mt-5">
            <SeasonalHeatmap data={historicalData.seasonalIntensity} />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200/80 bg-white/90 p-5 shadow-sm backdrop-blur-sm">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="text-xs font-medium text-slate-400">
              Showing {filteredEvents.length} of {historicalData.events.length}
            </span>
          </div>
